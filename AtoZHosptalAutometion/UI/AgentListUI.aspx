<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AgentListUI.aspx.cs" Inherits="AtoZHosptalAutometion.UI.AgentListUI" %>
    ...
    <script>
        ...
        function BindGridview() {
            $.ajax({
                ...
                url: "AgentListUI.aspx/BindGridview",
                data: "{}",
                dataType: "json",
                success: function (data) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        $("#gvDetails").append('<tr><td>' + count + '</td><td>' + result[i].Code + '</td><td>' + result[i].Name + '</td><td>' + result[i].Phone + '</td><td>' + result[i].Address + '</td><td> <img src=delete.png onclick=deleterecords(' + result[i].Id + ')> </td></tr>');
                        count += 1;
                    }                   
                },
                error: function (data) {
        ...
        function deleterecords(id) {
           // if($.conf)
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "AgentListUI.aspx/DeleteItem",
                data: "{'id':'" + id + "'}",
                dataType: "json",
                success: function (data) {
                    if (data.d == 'true')
                ...
                error: function (data) {
                    var r = data.responseText;
                    var errorMessage = r.Message;
                    alert('You are not eligible to get this service');
                }
            });
        }
